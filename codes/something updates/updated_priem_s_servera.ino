...
const char* baseUrl = "http://192.168.63.101";

// Определим переменные для статусов и расстояний
String status[3];
int distance[3];
bool attentionState = false; // Переменная для хранения состояния attention
...
void loop() {
  if (WiFi.status() == WL_CONNECTED) {
    for (int i = 1; i <= 2; i++) {
      ...
      String attentionURL = String(baseUrl) + "/attention"; // URL для получения состояния attention
    ...
    // Получение состояния attention
    HTTPClient httpAttention;
    String attentionURL = String(baseUrl) + "/attention";
    httpAttention.begin(attentionURL);
    int httpResponseCodeAttention = httpAttention.GET();
    if (httpResponseCodeAttention > 0) {
      String attentionResponse = httpAttention.getString();
      attentionState = (attentionResponse == "HIGH"); // Сохраняем состояние attention
      Serial.println("Состояние Attention: " + attentionResponse);
    } else {
      Serial.print("Ошибка при выполнении запроса состояния Attention: ");
      Serial.println(httpResponseCodeAttention);
    }
    httpAttention.end();
    ...
    // Управление сигналами на основе статуса и состояния attention
    for (int i = 0; i < 3; i++) {
      controlSignals(i);
    }

    // Управление пином attention
    digitalWrite(attention, attentionState ? HIGH : LOW);
  } else {
