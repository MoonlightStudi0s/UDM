...
IPAddress staticIP(192, 168, 188, 101);  // Статический IP-адрес
IPAddress gateway(192, 168, 188, 43);    // Основной шлюз
IPAddress subnet(255, 255, 255, 0);      // Маска подсети

WebServer server(80);

int num1 = 0, num2 = 0; // Переменные для хранения расстояний (теперь только два маячка)
...
void setup() {
    ...
    server.on("/status2", mayakStatus2); // Теперь это статус третьего маячка
    server.on("/pin2", mayakRasst2);    // Теперь это расстояние третьего маячка
    server.on("/attention", handleAttention); // Новый обработчик для пина attention
...
void handleRoot() {
    ...
    html += "<h1>Статус маячка 2:</h1>"; // Теперь это третий маячок
    html += "<p id='status2'>" + getPinStatus2() + "</p>";
    html += "<p id='pin2-status'>Расстояние 2: " + String(num2) + "</p>"; 

    html += "<h1>Состояние Attention:</h1>";
    html += "<p id='attention-status'>" + getAttentionStatus() + "</p>"; 
    ...
    html += "<script>setInterval(function() {fetch('/attention').then(response => response.text()).then(data => document.getElementById('attention-status').innerHTML = data);}, 1000);</script>";
...
// Функции для маячка 2 (теперь это третий маячок)
void mayakStatus2() {
...
// Обработчик для пина attention
void handleAttention() {
    String status = getAttentionStatus();
    server.send(200, "text/plain; charset=utf-8", status);
}
...
// Получение статуса для маячка 2 (теперь это третий маячок)
...
// Получение статуса для пина attention
String getAttentionStatus() {
    int attentionValue = digitalRead(attention);
    Serial.print("Состояние Attention: ");
    Serial.println(attentionValue == HIGH ? "HIGH" : "LOW");
    return attentionValue == HIGH ? "HIGH" : "LOW";
}
...
void parseData(String data) {
    ...
    int numReceived[2] = {0, 0}; // Массив для новых значений (теперь только два маячка)

    for (int i = 0; i < 2; i++) { // Ожидаем 2 числа
        if (endIndex == -1) {
